...
import com.github.kr328.clash.common.constants.Default_Config_Content
...
class NewProfileActivity : BaseActivity<NewProfileDesign>() {
                        ...
                        is NewProfileDesign.Request.Create -> {
                            withProfile {
                                ...
                                val uuid: UUID? = when (val p = it.provider) {
                                    ...
                                    is ProfileProvider.WebUI ->
                                        create(Profile.Type.WebUI, name, "", Default_Config_Content)
                                ...
                                if (uuid != null)
                                    launchProperties(uuid, it.provider)
                            }
                        }
    ...
    private suspend fun launchProperties(uuid: UUID, provider: ProfileProvider ) {

        val r = if (provider is ProfileProvider.WebUI) {
            startActivityForResult(
                ActivityResultContracts.StartActivityForResult(),
                WebPropertiesActivity::class.intent.setUUID(uuid))
    ...
    private suspend fun queryProfileProviders(): List<ProfileProvider> {
        return withContext(Dispatchers.IO) {
            ...
            listOf(ProfileProvider.File(self), ProfileProvider.Url(self), ProfileProvider.WebUI(self)) + providers
        }
    }
}
