...
class NetworkSettingsDesign(
    ...
    init {
        ...
        val screen = preferenceScreen(context) {
            ...
            editableInit(
                value = srvStore::externalPort ,
                adapter = NotNullTextAdapter.Port,
                default = 9090,
                title = R.string.external_controller_port,
                error = "range 1204 - 65536",
                validator = ValidatorRangePort,
                configure = vpnDependencies::add
            )

            editableInit(
                value = srvStore::externalSecret,
                adapter = NotNullTextAdapter.String,
                default = "clash",
                title = R.string.secret,
                error = "secret not allow empty",
                configure = vpnDependencies::add
            )

            switch(
                value = srvStore::startWithController,
                title = R.string.start_with_controller,
                summary = R.string.start_with_controller_summary,
                configure = vpnDependencies::add
            )
