...
JNIEXPORT void JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeLoadW(JNIEnv *env, jobject thiz,
                                                          jobject completable, jstring path, jint port, jstring secret, jboolean withBoard) {
    TRACE_METHOD();
    ...
    loadw(_completable, _path, port, _secret, withBoard);
}

//JNIEXPORT void JNICALL
//Java_com_github_kr328_clash_core_bridge_Bridge_nativeStartController(JNIEnv *env, jobject thiz,
//                                                           jobject completable, jstring pending,
//                                                           jstring processing, jstring imported, jint port, jstring secret) {
//    TRACE_METHOD();
//    jobject _completable = new_global(completable);
//    scoped_string _pending = get_string(pending);
//    scoped_string _processing = get_string(processing);
//    scoped_string _imported  = get_string(imported);
//    scoped_string _secret = get_string(secret);
//    startReadBoard(port, _secret);
//}

JNIEXPORT void JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeStartWriteUi(JNIEnv *env, jobject thiz,
                                                           jobject uiCallback,
                                                           jstring processing, jint port, jstring secret,
                                                           jstring uuid, jstring name, jint pType,
                                                           jstring source, jlong interval) {
    TRACE_METHOD();
    jobject _uiCallback = new_global(uiCallback);
    scoped_string _processing = get_string(processing);
    scoped_string _secret = get_string(secret);
    scoped_string _uuid = get_string(uuid);
    scoped_string _name = get_string(name);
    scoped_string _source = get_string(source);
    startWuiListener(_uiCallback, _processing, port, _secret, _uuid, _name, pType, _source, interval);
}

JNIEXPORT void JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeStopListener(JNIEnv *env, jobject thiz) {
    TRACE_METHOD();
    stopListener();
}

JNIEXPORT void JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeStopWriteListener(JNIEnv *env, jobject thiz) {
    TRACE_METHOD();
    stopWriteListener();
}
...
static jmethodID m_ui_callback_report;
...
static void call_ui_callback_report_impl(void *ui_callback, const char *status_json) {
    TRACE_METHOD();
    ...
    (*env)->CallVoidMethod(env,
                           (jobject) ui_callback,
                           (jmethodID) m_ui_callback_report,
                           (jstring) _status_json);
}
...
JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass c_ui_callback = find_class("com/github/kr328/clash/core/bridge/UiCallback");
    ...
    m_fetch_callback_complete = find_method(c_fetch_callback, "complete",
                                            "(Ljava/lang/String;)V");
    m_ui_callback_report = find_method(c_ui_callback, "report",
                                       "(Ljava/lang/String;)V");
    ...
    ui_report_func = &call_ui_callback_report_impl;
