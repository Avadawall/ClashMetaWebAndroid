...
object Clash {
    ...
    fun loadw(path: File, port: Int, secret: String, withBoard: Boolean): CompletableDeferred<Unit> {
        return CompletableDeferred<Unit>().apply {
            Bridge.nativeLoadW(this, path.absolutePath, port, secret, withBoard)
        }
    }
    ...
    fun startWriteUi(processing: File, port: Int, secret: String,
                     uuid: String, name:String, pType: Int, source: String, interval: Long,
                     cb: UiCallback ) {
        Bridge.nativeStartWriteUi(cb, processing.absolutePath, port, secret, uuid, name, pType, source, interval)
//        Bridge.nativeStartWriteUi(object: UiCallback {
//            override fun report(config: String) {
//                Log.w("startWriteUi report logger $config")
//            }
//        }, processing.absolutePath, port, secret)
//        return CompletableDeferred<Unit>().apply {
//            Bridge.nativeStartWriteUi(this, processing.absolutePath, port, secret)
//        }
    }

    fun stopListener() {
        Bridge.nativeStopListener()
    }

    fun stopWriteListener() {
        Bridge.nativeStopWriteListener()
    }
